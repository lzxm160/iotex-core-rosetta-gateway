...
printf "${GRN}### Waiting for block height to 20...${OFF}\n"
sleep 200

printf "${GRN}### Validating Rosetta gateway implementation...${OFF}\n"
./rosetta-cli check --lookup-balance-by-block=false --end 20 --bootstrap-balances ./bootstrap_balances.json --block-concurrency 8
