...
rosetta_cli_tar=$(wget --quiet --show-progress --progress=bar:force:noscroll -O rosetta-cli-${ROSETTA_CLI_RELEASE}.tar.gz https://github.com/coinbase/rosetta-cli/archive/v${ROSETTA_CLI_RELEASE}.tar.gz)
iotex_core_tar='wget --quiet --show-progress --progress=bar:force:noscroll -O iotex-core-${IOTEX_SERVER_RELEASE}.tar.gz https://github.com/iotexproject/iotex-core/archive/${IOTEX_SERVER_RELEASE}.tar.gz'
